...
public class ListReferenceOtherController extends AbstractListController{
	...
	public class RefHeaderDataModel extends StdRequestDataModel<RefHeader>{
		...
		protected List<RefHeader> internalList(SequenceRange seqRange,
			...
			if (StringUtils.isEmpty(name))
				name=null;
			List<RefHeader> lst=refBook.listRefHeaders(Partner.SYSTEM, name,false);
			return lst;
		}
		...
		protected int internalRowCount() throws WebAppException {
			if (StringUtils.isEmpty(name))
				name=null;
			return refBook.countRefHeaders(Partner.SYSTEM, name,false);
		}
