...
public class EditCreditRequestVerificatorController extends AbstractSessionController implements Serializable{

	
	/**
	 * 
	 */
	private static final long serialVersionUID = -2515595718990828049L;
	...
	private static final Logger logger = LoggerFactory.getLogger(EditCreditRequestVerificatorController.class.getCanonicalName());
	...
	public void saveAll(){
        ...
        try {
			...
			if (getCanEditCreditRequest()) {
				kassaBean.saveCreditRequest(creditRequest.getEntity());
			}
			...
			int id=prmCreditRequestId;
			//если заявка недозаполненная, создаем запись пользователя
			...
			if (user==null){
				user=userBean.findUserByPeopleId(peopleMain.getId());
				//у человека тоже нет логина
				if (user==null){
				    log.info("Пользователя не было по заявке "+prmCreditRequestId);
			    
                    PeopleMainEntity pme=peopleDAO.getPeopleMainEntity(peopleMain.getId());
                    user=userBean.addUserClient(pme, login);
                
                    log.info("Добавили пользователя по заявке "+prmCreditRequestId);
                    ...
                    creditRequestEntity=kassaBean.newCreditRequest(creditRequestEntity,
                		...
                		stake,"",null,GenUtils.genUniqueNumber(new Date(), i, stake * 100),null,null,RefCreditRequestWay.ONLINE,null);
                    creditRequest=new CreditRequest(creditRequestEntity);
             
                    generateAgreement(new Date());
               
                    //сохранили данные
                    id = kassaBean.saveCreditRequest(creditRequest, "127.0.0.1", "");
                    log.info("Сохранили заявку "+prmCreditRequestId);
				}
            
			}//end нет пользователя с таким логином
			
			
			//если заявка заполнена, можно идти дальше
			if (creditRequest.getStatus().getId()==CreditStatus.FILLED){
				List<WorkflowObjectStateDef> lstAct = workflow.getProcCRWfActions(id, null, true);
				WorkflowObjectStateDef taskDef = WorkflowUtil.findWFState(lstAct, new StateRef("*","*","taskVerificatorEdit"));
				if (taskDef == null) {
					// TODO исправить
					String msgMore = lstAct.get(0).getActions().get(0).getSignalRef();            	
					workflow.goProcCR(id, msgMore, null);
				}
			}
			//пишем лог
			CreditRequestEntity crequest=crDAO.getCreditRequestEntity(creditRequest.getId());
			if (crequest!=null){
			    eventLog.saveLog("localhost",  eventLog.getEventType(EventType.INFO).getEntity(), eventLog.getEventCode(EventCode.MANAGER_CALL).getEntity(), "редактирование заявки верификатором ", crequest, null, userCtl.getUser().getEntity(),null,"","","","");
			}
