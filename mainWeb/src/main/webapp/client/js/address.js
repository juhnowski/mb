/**
 * Created by PARFENOV on 25.05.2015.
 */
!function ($) {
    "use strict"; // jshint ;_;
    ...
    Address.prototype = {
        ...
        loadChildren: function (level, parentValue, isEdit) {
            var that = this;
            var $parent = this.$element.find('[data-address="level' + level + '"]').parents('div.row:first');
                ...
                success: function (res) {
                    if (res.length > 0) {
                        ...
                        if ($nextSelect.length < 1) {
                            $nextSelect = $('<div class="row">' +
                                '<div class="col first"><label>'+(levelName ? levelName : 'Выберите')+'</label></div>' +
                                '<div class="col focus">' +
                                '<select required="required" class="required" data-address="level'+ nextLevel +'" '+ ( isEdit != undefined ? '' : '') +'>' +
                                optionsHtml+
                                '</select>' +
                                '</div>' +
                                '</div>');
                            ...
                            $nextSelect.closest('.row').find('.col.first').html('<label>' +levelName + '</label>');
                        }
                        $nextSelect.on('change', function (e) {
                            $(this).find('.placeholder').remove();
                            var selected = $(this).find(':selected');
                            if (selected && selected.attr('data-levelName')) {
                                $(this).closest('.row').find('.col.first').html('<label>' + selected.attr('data-levelName') + '</label>');
                            } else {
                                $(this).closest('.row').find('.col.first').html('<label>' +label + '</label>');
                            }
                        ...
                        $nextSelect.on("select2:open", function () {
                            var selected = $(this).find(':selected');
                            if(!selected.hasClass('not-valid-row')){
                                selected.closest('.row').addClass('focus').removeClass('red');
                            }
                        });
                        $nextSelect.on("select2:close", function () {
                            var selected = $(this).find(':selected');
                            selected.closest('.row').removeClass('focus').removeClass('red');
                            if (!selected.hasClass('not-valid-row')) {
                                if (selected.val() == null || selected.val() == '') {
                                    selected.closest('.row').addClass('red')
                                } else {
                                    selected.closest('.row').addClass('green');
                                }
                            }
                        });
